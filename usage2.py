        ...
        html.Div(
            [
                ...
                html.Img(
                    ...
                    ,
                    style={'width': '300px', 'height': '85px', 'backgroundPosition': 'center', 'paddingTop': '10px'}
...
def submitGeneToIlincsMethod(gene_name, link, topConcordance, input_df):
    ...
    for item in list_of_concordance:
        ...
        total_cell.append('CellLine: ' + item[2])
    ...
    return json.dumps(output, default=to_serializable)
...
def make_network_obj(list_of_contents, gene, num_compounds, list_of_names, list_of_dates):
    ...
    if gene is not '':
        ...
        # network_obj = json.dumps(network_obj)
    ...
    if chem_dropdown_values is not None:
        ...
        try:
            ...
            for item in chem_dropdown_values:
                ...
                url2 = "http://www.ilincs.org/api/SignatureMeta?filter=%7B%22where%22%3A%7B%22lincspertid%22%3A%22{}%22%7D%2C%22limit%22%3A1%7D".format(
                    lsm)
                ...
                c1.append(html.A(lsm, href="http://www.ilincs.org/ilincs/perturbagen/compound/{}".format(lsm),
                                 target="_blank",
...
def display_molecule(hoverData):
    ...
    if hoverData is not None:
        try:
            ...
            url = "http://www.ilincs.org/api/CompoundMOAs?filter=%7B%22where%22%3A%7B%22lincsPertID%22%3A%22{}%22%7D%7D".format(
                lsm)
    ...
    if hoverData is not None:
        try:
            ...
            for item in data:
                # "secondChemID":"LSM-6474", "compound":"ST013886", "pubchemCID":450, "similarity":1
                if item['secondChemID']:
                    fig.append(html.P("LSMID: {} / ".format(item['secondChemID']),
                                      style={"display": "inline-block", "marginTop": "5px", "marginBottom": "2px"}))
                    # fig.append(html.P(item['secondChemID']))
                # if item['compound']:
                #     fig.append(html.P("compound: {} / ".format(item['compound']), style={"display": "inline-block","marginTop": "5px", "marginBottom": "2px"}))
                #     # fig.append(html.P(item['compound']))
                if item['pubchemCID']:
                    fig.append(html.P("pubchemID: {} / ".format(item['pubchemCID']),
                                      style={"display": "inline-block", "marginTop": "5px", "marginBottom": "2px"}))
                    # fig.append(html.P(item['pubchemCID']))
                if item['similarity']:
                    fig.append(html.P("similarity: {} ".format(item['similarity']),
                                      style={"display": "inline-block", "marginTop": "5px", "marginBottom": "2px"}))
                    # fig.append(html.P(item['similarity']))
                fig.append(html.Hr(
                    style={"paddingTop": "2px", "paddingBottom": "2px", "marginTop": "2px", "marginBottom": "2px"}))
